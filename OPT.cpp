	...
	while (seqN--) {
		...
		if (seqN<=flag){
            ...
            for(int t=0;t<tasks.size();t++){
                if(tasks[t].flow<1 && tasks[t].endTime>node.begTime)
                    temptasks.push_back(tasks[t]);
            }
            for(int t=0;t<workers.size();t++){
                if(workers[t].flow<workers[t].cap && workers[t].endTime>node.begTime)
...
//    int sumUsedMemory=0, sumWorker=0;
    ...
    for(int i=0;i<1;i++) {
        edgeFileName.clear();
        edgeFileName += "./data/real/real2/2000/data_200";
...
#else
        printf("ONRTA-Greedy %d %.6lf %.6lf \n", i+1, utility,  usedTime );
