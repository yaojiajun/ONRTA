            ...
            if (!isSecondHalf) {
                ...
                if (T_delta.size() >= W_delta.size()) {
                    ...
                    for (int i = Wdelsz; i < Wsz; ++ i) {
                        ...
                        if (workers[workerId].flow<workers[workerId].cap){
                            taskId = chosenNextTask(tasks, workers[workerId]);
                            if (workerId >= 0 && taskId >= 0) {
                                addOneMatch(tasks[taskId], workers[workerId]);
                                matched_worker++;
                                response_time=nd.begTime-tasks[taskId].begTime;
                                avg_response_time=avg_response_time+response_time;
                                cout << "3<" << tasks[taskId].id << "," << workers[workerId].id << ">" << utility << " "
                                     << matched_worker << "," << avg_response_time<< endl;
                            }
                        }
                    }
                    for (int i = Tdelsz; i < Tsz; ++ i) {
                        ...
                        if (tasks[taskId].flow<tasks[taskId].cap){
                            workerId = chosenNextWorker(workers, tasks[taskId]);
                            if (workerId >= 0 && taskId >= 0) {
                                addOneMatch(tasks[taskId], workers[workerId]);
                                matched_worker++;
                                response_time=nd.begTime-tasks[taskId].begTime;
...
int main(int argc, char* argv[]) {
...
#else
        printf("TOGA-OP %d %.6lf %.6lf %d %d \n", i+1, utility, usedTime, usedMemory, matched_worker);
    ...
    printf("TOGA-OP  %.6lf %.6lf %d %.d\n",  sumUtility/100, sumUsedTime/100, sumUsedMemory/100,  sumWorker/100);
    return 0;
}
